...
namespace Dashboard.Controllers {
    public class HomeController : Controller {
        ...
        public HomeController(ILogger<HomeController> logger,
            ...
            IEntityRepository entityRepository, IDeviceRepository deviceRepository, ITemperatureRepository temperatureRepository, IServiceRepository serviceRepository, IHumidityRepository humidityRepository, ILightRepository lightRepository, IRuleRepository ruleRepository)
        {
            _logger = logger;
            _context = context;
        ...
        [HttpPost]
        public IActionResult DefinedDeviceView(DeviceModel input)
        {
           // if (!ModelState.IsValid)
               // return View(input);


            var entity = new DeviceControl()
            {
                name = input.Name,
                opt = input.Control_Status_Bool == true ? "on" : "off" 
            };
            
           // var result = _deviceRepository.Control(new DeviceControl()
           // {
           //     name = input.Name,
           //     opt = input.Control_Status_Bool == true ? "on" : "off" 
           // });
        ...
        public IActionResult ControlDeviceView(string name)
        {
           
            var result = _deviceRepository.GetAll();

            var device = result.FirstOrDefault(_ => _.Name.Contains(name));
            return View(device);
        ...
        {
            var result = _temperatureRepository.GetAll();
        ...
        {
            var result = _humidityRepository.GetAll();
        ...
        {
            var result = _lightRepository.GetAll();
        ...
        // Rule
          public IActionResult DefinedRuleView()
        {            
            var result = _ruleRepository.GetAll(0,100);
            return View(result);
        }

        public IActionResult CreateRuleView()
        {
            return View();
        }

        [HttpPost]
        public IActionResult CreateRuleView(RuleModel model)
        {
            
            _ruleRepository.Create(model);
            return RedirectToAction("DefinedRuleView");
        ...
        public IActionResult EditRuleView(int id)
        {
            var result = _ruleRepository.GetAll(0,100);

            var model = result.FirstOrDefault(p => p.Id == id);
            if (model == null)
            {
                return NotFound();
            }
        ...
        [HttpPost]
        public IActionResult EditRuleView(RuleModel model)
        {
            _ruleRepository.Update(model);
